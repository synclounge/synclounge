          ...
          <v-flex xs12>
            ...
            >
              <v-layout row wrap justify-space-between align-center>
                ...
                <v-flex xs12>
                  <div
                    v-if="ptUsers.length != 1"
                    class="participant-count"
                  >{{ ptUsers.length }} people</div>
            ...
            >
              <v-layout row wrap justify-space-between align-center>
                <v-flex xs12 class="text-center">
                  ...
                  >Waiting for {{ hostUser().username }} to start</span>
                </v-flex>
              </v-layout>
            </v-card>
          </v-flex>
                  ...
                  <v-tooltip bottom color="rgb(44, 44, 49)" multi-line class="userlist">
                    <span>
                      <v-list-item-title>
                        ...
                        <span
                          style="opacity: 0.6"
                          v-if="hostUser().uuid === me.uuid"
                        >(you)</span>
                      </v-list-item-title>
                      <v-list-item-subtitle
                        style="opacity:0.6;color:white;font-size:70%"
                      >{{ getTitle(hostUser()) }}</v-list-item-subtitle>
                    </span>
                    Watching on {{ hostUser().playerProduct || 'Unknown Plex Client' }}
                    <span
                      v-if="plex.servers[hostUser().machineIdentifier]"
                    >
                      <br />
                      via {{ plex.servers[hostUser().machineIdentifier].name }}
                    </span>
                  </v-tooltip>
                </v-list-item-content>
                <v-list-item-action>
                  ...
                  <v-list-item-content>
                    <v-tooltip bottom color="rgb(44, 44, 49)" multi-line class="userlist">
                      <span>
                        ...
                        <v-list-item-subtitle
                          style="opacity:0.6;color:white;font-size:70%"
                        >{{ getTitle(user) }}</v-list-item-subtitle>
                      </span>
                      Watching on {{ user.playerProduct || 'Unknown Plex Client' }}
                      <span
                        v-if="plex.servers[user.machineIdentifier]"
                      >
                        <br />
                        via {{ plex.servers[user.machineIdentifier].name }}
                      </span>
                    </v-tooltip>
                  </v-list-item-content>
                  <v-list-item-action>
...
export default {
  ...
  methods: {
    ...mapActions(['updatePartyPausing', 'sendPartyPause', 'SET_RIGHT_SIDEBAR_OPEN']),
    isHost(user) {
