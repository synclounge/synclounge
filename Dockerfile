# build environment
FROM --platform=$BUILDPLATFORM node:21.3.0-alpine3.18 as build-stage
...
FROM node:21.3.0-alpine3.18 as dependency-stage
RUN mkdir /app && chown -R node:node /app
WORKDIR /app
RUN apk add --no-cache python3 make g++
USER node
...
FROM node:21.3.0-alpine3.18 as production-stage
RUN mkdir /app && chown -R node:node /app
WORKDIR /app
