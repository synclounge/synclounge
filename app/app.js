...
ipcMain.on('plextv-signin', function(event, username, password) {
  ...
  plex.doStandardLogin(username, password, function(result) {
    if (result == 201) {
      global.storage.set('plex-together-settings', {
        'plextvtoken': plex.user.authToken
      }, function(error) {
        if (error) throw error;
      });
...
ipcMain.on('home-tab-initialize', function(event) {
  ...
  event.sender.send('home-tab-initialize-result', plex.user.username, plex.clients, plex.servers);
...
ipcMain.on('home-tab-me-avatar', function() {
    ...
    request(options, function(error, response, body) {}).pipe(fs.createWriteStream(path))
      .on('close', function() {
...
ipcMain.on('ptuser-thumb-download', function(event, userData) {
  ...
  request(userData.avatarUrl, function(error, response, body) {}).pipe(fs.createWriteStream(path))
    .on('close', function() {
...
ipcMain.on('ptmain-thumb-download', function(event) {
  ...
  request(plex.user.thumb, function(error, response, body) {}).pipe(fs.createWriteStream(path))
    .on('close', function() {
      log.info('downloading done from ' + path)
...
ipcMain.on('connect-to-ptserver', function(event, address) {
  global.socket = global.io.connect(address, {
